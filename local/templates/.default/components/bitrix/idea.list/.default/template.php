...
if(count($arResult["POST"] ?? [])>0)
{
    ...
    pretty_print($arStatusList,'$arStatusList');
		...
		<div class="blog-qtl<?if(in_array($CurPost["PUBLISH_STATUS"], array(BLOG_PUBLISH_STATUS_READY, BLOG_PUBLISH_STATUS_DRAFT))):?> blog-post-hidden<?endif;?>">
			<div class="blog-qtr">
				<div class="blog-idea-body">
                    ...
                    <p class="select-moderator" id="bp-<?=$CurPost["ID"]?>">
                        <a href="#">Выбрать модератора</a>
                    </p>

                    <!--<div class="select-moderator" id="bp-<?php /*= $arResult['Post']['ID'] */?>">-->
                    <button class="ui-btn ui-btn-light" onclick="openModeratorsDialog(this)">Выбрать модераторов</button>
                    <button  data-users-ids="" class="ui-btn ui-btn-primary" onclick="createTasks(this)">
                        Создать задачу
                    </button>
...
<script>
    // Функция для открытия диалога выбора модераторов
    function openModeratorsDialog(buttonNode) {
        BX.Runtime.loadExtension('ui.entity-selector').then((exports) => {
            const { Dialog } = exports;
            const dialog = new Dialog({
                targetNode: buttonNode, // Кнопка, к которой привязан диалог
                context: 'idea_moderators', // Контекст для сохранения выбора
                multiple: true, // Множественный выбор
                enableSearch: true,
                entities: [
                    {
                        id: 'user', // Сущность "Пользователи"
                    }
                ],
                events: {
                    'Item:onSelect': (event) => {
                        // Можно обрабатывать выбор динамически, если нужно
                    }
                }
            });
    ...
    function createTasks(selectedUsers) {
        const userIds = selectedUsers.map(item => item.getId());
