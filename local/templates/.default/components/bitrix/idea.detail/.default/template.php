            ...
            <div class="blog-qtr">
                <div class="blog-idea-body">
                    <div class="idea-owner">
                        <div class="bx-idea-condition-description status-color-<?= mb_strtolower($arStatusList[$arResult["POST_PROPERTIES"]["DATA"]["UF_STATUS"]["VALUE"]]["XML_ID"]); ?>">
                            <div <? if ($arResult["IDEA_MODERATOR"]): ?>class="status-action idea-action-cursor"
                                 onclick="JSPublicIdea.ShowStatusDialog(this, '<?= $arResult["Post"]["ID"] ?>');"
                                 id="status-<?= $arResult["Post"]["ID"] ?>"<?endif;
                            ?>><?= htmlspecialcharsbx($status) ?></div>
                        </div>
                        <?= GetMessage("IDEA_INTRODUCED_TITLE") ?> <img class="idea-user-avatar"
                                                                        src="<?= $arResult["AUTHOR_AVATAR"][$arResult["arUser"]["ID"]]["src"] ?>"
                                                                        align="top">
                        <?
                    ...
                    <div class="post-title"><h2><a
                                    title="<?= $arResult["Post"]["TITLE"] ?>"><?= $arResult["Post"]["TITLE"] ?></a></h2>
                    </div>
                    <!--<div class="select-moderator" id="bp-<?php /*= $arResult['Post']['ID'] */?>">-->
                        <?php
                       echo $postId = (int)$arResult['Post']['ID'];
                        ...
                        <div
                                class="select-moderator"
                                id="bp-<?= $postId ?>"
                                data-post-id="<?= $postId ?>"
                        >
                            <input
                                    type="hidden"
                                    class="selected-user-ids"
                                    name="USER_IDS_JSON"
                                    value="[]"
                            >
                        ...
                        <div class="idea-post-meta">
                            <div class="idea-post-meta-util">
                                <? if ($arResult["urlToHide"] <> ''): ?>
                                    <a href="javascript:void(0)"
                                       onclick="if(confirm('<?= GetMessage("BLOG_MES_HIDE_POST_CONFIRM") ?>')) window.location='<?= $arResult["urlToHide"] . "&" . bitrix_sessid_get() ?>'"><span><?= GetMessage("BLOG_MES_HIDE") ?></span></a>
                                <? elseif ($arResult["urlToShow"] <> ''): ?>
                                    <a href="javascript:void(0)"
                                       onclick="if(confirm('<?= GetMessage("IDEA_MES_SHOW_POST_CONFIRM") ?>')) window.location='<?= $arResult["urlToShow"] . "&" . bitrix_sessid_get() ?>'"><span><?= GetMessage("IDEA_MES_SHOW") ?></span></a>
                                <? endif; ?>
                                <? if ($arResult["urlToEdit"] <> ''): ?>
                                    <a href="<?= $arResult["urlToEdit"] ?>"><span
                                                class="blog-post-link-caption"><?= GetMessage("BLOG_BLOG_BLOG_EDIT") ?></span></a></span>
                                <? endif; ?>
                                <? if ($arResult["urlToDelete"] <> ''): ?>
                                    <a href="javascript:void(0)"
                                       onclick="if(confirm('<?= GetMessage("BLOG_MES_DELETE_POST_CONFIRM") ?>')) window.location='<?= $arResult["urlToDelete"] . "&" . bitrix_sessid_get() ?>'"><span><?= GetMessage("BLOG_BLOG_BLOG_DELETE") ?></span></a>
                                <? endif; ?>
                            </div>
                            <div class="idea-post-meta-util-subscribe">
                                <? if (in_array(CIdeaManagmentEmailNotify::SUBSCRIBE_IDEA_COMMENT . $arResult["Post"]["ID"], $arResult["USER_IDEA_SUBSCRIBE"])): ?>
                                    <a class="idea-post-unsubscribe"><span
                                                class="idea-post-subscribe-<?= $arResult["Post"]["ID"] ?>"><?= GetMessage("IDEA_POST_UNSUBSCRIBE") ?></span></a>
                                <? else: ?>
                                    <a class="idea-post-subscribe"><span
                                                class="idea-post-subscribe-<?= $arResult["Post"]["ID"] ?>"><?= GetMessage("IDEA_POST_SUBSCRIBE") ?></span></a>
                                <? endif; ?>
                            </div>
                            <br clear="both"/>
                        </div>
        ...
        <div class="tag-tr">
            <div class="tag-block">
						...
						<span class="main-tag-category"><?
                            if ($arResult["IDEA_CATEGORY"]["NAME"] !== false) {
                                if ($arResult["IDEA_CATEGORY"]["LINK"] === false)
                                    ...
                                    ?><a
                                    href="<?= $arResult["IDEA_CATEGORY"]["LINK"]; ?>"><?= $arResult["IDEA_CATEGORY"]["NAME"]; ?></a><?
                                }
                            } ?></span>
                ...
                <span class="post-comment"><a
                            href="<?= $arResult["Post"]["urlToPost"] ?>#comments"><?= GetMessage("IDEA_POST_COMMENT_CNT") ?>: <?= (intval($arResult["Post"]["NUM_COMMENTS"]) - $cntOfficial); ?></a></span>
                <br clear="both"/>
            </div>
        </div>
    </div>
...
<script>
    BX.ready(function () {
        ...
        if (!root || root.dataset.initialized === 'Y') {
            return;
        }

        root.dataset.initialized = 'Y';

        const selectButton = root.querySelector('.select-user-button');
        const createTasksButton = root.querySelector('.create-tasks-button');
        const userIdsInput = root.querySelector('.selected-user-ids');
        const userInfo = root.querySelector('.selected-user-info');
        ...
        function renderSelectedUsers() {
            BX.cleanNode(userInfo);

            selectedUsers.forEach(function (item) {
                userInfo.appendChild(createUserNode(item));
            });
            ...
            selectButton.textContent = userIds.length > 0
                ? 'Изменить выбор'
                : 'Выбрать модераторов';
        ...
        BX.bind(selectButton, 'click', function () {
            userDialog.show();
        });
        ...
        BX.bind(createTasksButton, 'click', function () {
            ...
            createTasksButton.disabled = true;
