...
if (!CSocNetFeaturesPerms::CanPerformOperation($GLOBALS["USER"]->GetID(), SONET_ENTITY_USER,
    $arResult["VARIABLES"]["user_id"], "search", "view", CSocNetUser::IsCurrentUserModuleAdmin())) {
...
if (
    ...
    $feature = "search";
    $arEntityActiveFeatures = CSocNetFeatures::GetActiveFeaturesNames(SONET_ENTITY_USER,
        $arResult["VARIABLES"]["user_id"]);
    $strFeatureTitle = ((array_key_exists($feature,
            $arEntityActiveFeatures) && $arEntityActiveFeatures[$feature] <> '') ? $arEntityActiveFeatures[$feature] : GetMessage("USER_CONTENT_SEARCH_TITLE"));
...
<? $APPLICATION->IncludeComponent("bitrix:search.page", "tags_icons_user", array(
    ...
    "TAGS_URL_SEARCH" => CComponentEngine::MakePathFromTemplate($arParams["~PATH_TO_USER_CONTENT_SEARCH"],
