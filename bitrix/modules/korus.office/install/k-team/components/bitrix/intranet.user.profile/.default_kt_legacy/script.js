;(function () {
    ...
    BX.ready(function () {
        const businessSignsHintTooltip = BX.UI.Hint.createInstance({
            popupParameters: {
                className: 'intranet-user-profile-business-signs-hint-popup'
            }
        });

        let jsBusinessSignsSlider = BX('js-business-signs-slider');

        if (jsBusinessSignsSlider !== null) {
            businessSignsHintTooltip.init(jsBusinessSignsSlider);

            if (jsBusinessSignsSlider.classList.contains('slider-mode')) {
                new Glide('#js-business-signs-slider', {
                    dragThreshold: false,
    ...
    namespace.Manager.prototype = {
        ...
        initAvatarLoader: function () {
            ...
            BX.bind(BX('intranet-user-profile-photo-camera'), "click", function () {
                resCamera.show('camera');
            });
            BX.bind(BX('intranet-user-profile-photo-file'), "click", function () {
                resCamera.show('file');
            });
...
let phonePersonalEdit = function () {

    mainBlock$ = $('.wrap-edit-fields-js');

    block$ = $('.wrap-edit-fields-js .ui-entity-editor-content-block[data-cid="UF_MOB_PHONE_PER"]');

    if (block$.length > 0) {

        $(document).on('click', '.phone-block-icon', function () {

            let visiblePhone = parseInt(mainBlock$.attr('data-phone-visible'), 10);

            let newVisiblePhone = visiblePhone > 0 ? 0 : 1;

            $.post(
                '',
                {
                    action: 'changePhoneVisibility',
                    visibility: newVisiblePhone,
                },
                function (data) {
                    mainBlock$.attr('data-phone-visible', newVisiblePhone).toggleClass('phone-not-visible');
