    ...
    BX.load(['/bitrix/js/ui/entity-editor/js/editor.js'], function () {
        if (typeof BX.Intranet.UserProfile.EntityEditorTimezone === "undefined") {
            ...
            BX.Intranet.UserProfile.EntityEditorTimezone.prototype.layout = function (options) {
                ...
                {
                    var getTimezoneFormatted = function (value) {
                        var formatTimezone = function (format, timezone) {
                            var options = {timeZoneName: format == 'short' ? 'short' : 'long'};

                            if (timezone) options['timeZone'] = timezone;

                            var strDate = new Intl.DateTimeFormat([], options).format(new Date());
                        ...
                        short = formatTimezone('short', timeZone);
                        long = formatTimezone('long', timeZone);
                    ...
                    containerPropsTimeZone = {props: {className: "ui-entity-editor-content-block-text"}};
                    if (isHtmlOption) {
                        containerPropsTimeZone.html = getTimezoneFormatted(value);
                    } else {
                        containerPropsTimeZone.text = getTimezoneFormatted(value);
                    }

                    this._textTimeZone = BX.create("div", containerPropsTimeZone);

                    this._innerWrapper = BX.create("div",
                        {
                            props: {className: "ui-entity-editor-content-block"},
                            children: [this._textTimeZone]
                        }
                    );
